...
const oldSearch=`function searchPulse(){
   const q=pulseInput.value.trim().toUpperCase(); if(!q)return;
   let a=assets.find(x=>x.k===q||x.n.toUpperCase()===q||x.n.toUpperCase().includes(q));
   if(a){
     pulseSearchResult.innerHTML=\`<div class="card pulse-company-card" style="padding:16px" onclick="openCompanyResearch('${'${a.k}'}')">
...
const newSearch=`function searchPulse(){
   const q=pulseInput.value.trim().toUpperCase(); if(!q)return;
...
if(!html.includes(oldSearch))throw new Error('searchPulse block not found');
...
console.log('Patched direct equity routing, market-instrument Pulse results and refresh state restoration.');
