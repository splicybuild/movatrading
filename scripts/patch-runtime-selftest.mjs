...
const start=html.indexOf('<script id="mova-preview-followup-fixes-v2">');
const end=start>=0?html.indexOf('</script>',start):-1;
must('final follow-up v2 runtime exists',start>=0&&end>start);
const openEnd=html.indexOf('>',start);
const js=html.slice(openEnd+1,end);
new Function(js);
console.log('PASS: final follow-up v2 runtime parses as JavaScript');
...
must('News Top 10 binds to known News input',html.includes("id==='newsSearchInput'"));
