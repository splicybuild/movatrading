...
const runtime=`<script id="mova-preview-followup-fixes-v3">(function(){
  ...
  function setWatch(ticker,on){ticker=sym(ticker);if(!ticker)return;var list=watched().filter(function(x){return x!==ticker});if(on)list.push(ticker);if(!on&&list.length===0){try{sessionStorage.setItem(MODE_KEY,'watchlist')}catch(e){}}writeWatch(list);refreshWatch(true)}
  ...
  function syncTopTicker(){
    var hit=tickerRoot();if(!hit)return;var list=watched(),active=watchMode(hit),empty=hit.root.querySelector('[data-mova-watch-empty="1"]');
    ...
    var price=/[$£€]\\s*[0-9]/;
    Array.from(hit.root.querySelectorAll('div,button,a')).forEach(function(el){if(el===hit.button||el===hit.trending||el===empty)return;var r=el.getBoundingClientRect(),t=el.innerText||'';if(!price.test(t)||r.height<22||r.height>145||r.width<50||r.width>520)return;if(Array.from(el.children).some(function(c){return price.test(c.innerText||'')}))return;var s=cardSymbol(el);if(!s||!list.includes(s)){el.dataset.movaWatchPrevDisplay=el.style.display||'';el.dataset.movaWatchHidden='1';el.style.display='none'}});
    if(!list.length){if(!empty){empty=document.createElement('div');empty.dataset.movaWatchEmpty='1';empty.textContent='No saved markets';empty.style.cssText='padding:10px 14px;color:#8299aa;font-size:11px;font-weight:800;white-space:nowrap';hit.root.appendChild(empty)}}else if(empty)empty.remove();
  }
