...
const runtime=`<script id="mova-global-autocomplete-runtime">(function(){
  ...
  function classify(input){if(input.id==='headerSearch'||/Search MOVA/i.test(input.placeholder||''))return'header';if(input.id==='mnaAlertSymbol'||/AMD or GOLD/i.test(input.placeholder||''))return'alert';if(input.closest&&input.closest('#movaNativeAuth,#movaNativeAccount'))return'';if(/Search a stock/i.test(input.placeholder||''))return'news';if(inPulseSearch(input))return'pulse';return''}
...
console.log('MOVA autocomplete now covers header, News, Pulse and Alerts; account forms excluded; Alerts fields aligned.');
