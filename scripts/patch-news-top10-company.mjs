...
const css=`<style id="mova-news-top10-v1-style">
...
const runtime=`<script id="mova-news-top10-v1-runtime">(function(){
  ...
  function findInput(){
    const news=document.getElementById('news');if(!news)return null;
    return [...news.querySelectorAll('input')].find(i=>/search a stock/i.test(i.placeholder||''))||null;
  }
  function findButton(){
    const news=document.getElementById('news');if(!news)return null;
    return [...news.querySelectorAll('button')].find(b=>/find\s*top\s*10/i.test((b.textContent||'').trim()))||null;
  }
  ...
  async function runSearch(){
    ...
    }catch(e){grid.innerHTML='<div class="mova-news-search-status">Top stories could not be loaded right now. Please try again shortly.</div>'}
  }
  function bind(){
    const input=findInput(),button=findButton();if(!input||!button)return;
    if(button.dataset.movaTop10Bound==='1')return;
    button.dataset.movaTop10Bound='1';button.type='button';
    button.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();runSearch()});
    const form=button.closest('form')||input.closest('form');
    if(form&&!form.dataset.movaTop10Bound){form.dataset.movaTop10Bound='1';form.addEventListener('submit',function(e){e.preventDefault();runSearch()})}
    input.addEventListener('keydown',function(e){if(e.key==='Enter'){e.preventDefault();runSearch()}});
  }
  const mo=new MutationObserver(bind);
  function start(){bind();mo.observe(document.body,{subtree:true,childList:true})}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();
})();</script>`;
html=html.replace('</body>',runtime+'</body>');
writeFileSync(file,html);
console.log('MOVA News Find Top 10 company search wired without page refresh.');
