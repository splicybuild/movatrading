...
function bestPageLogo(html,base){
  ...
  for(const tag of imgs){
    ...
    if(!/logo|brand|wordmark/.test(hint))continue;
    const url=absoluteUrl(src,base);
    if(!url)continue;
    let score=50;
    if(/\.svg(?:\?|$)/i.test(url))score+=80;
    if(/logo/.test(hint))score+=30;
    if(/header|navbar|nav/.test(hint))score+=20;
    if(/footer/.test(hint))score-=10;
...
export default{async fetch(request){
  ...
  try{
    ...
    const pageLogo=bestPageLogo(html,finalUrl); const icon=bestIcon(html,finalUrl);
    ...
    return Response.json({
      ...
      logo:pageLogo||icon||fallbackLogo,
