...
async function companyNews(symbol,key,days=14){
  ...
  return (Array.isArray(rows)?rows:[]).slice(0,12).map(x=>({
    symbol,headline:clean(x.headline),title:clean(x.headline),summary:clean(x.summary),body:clean(x.summary),source:clean(x.source),
  ...
  try{
    ...
    if(mode==="watchlist"){
      ...
      for(const symbol of requested){
        if(["GOLD","SILVER","OIL"].includes(symbol))continue;
        try{items.push(...await companyNews(symbol,key,21));}
        catch(e){errors.push({symbol,error:e.message});}
      }
      return Response.json({source:"Finnhub company news",asOf:new Date().toISOString(),items:dedupe(items).slice(0,24),errors},
