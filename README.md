...
## Current build
**Version:** v104 — Visual Learning + Linked Signals + News Layout

### v104 changes
- Desktop News header actions were realigned: Home now sits on the left edge of the News content and Refresh news sits on the right.
- Hot / Research and Not / Caution assets in Pulse → Hot or Not are now clickable from the company/asset name and open the matching asset research page.
- Added real-life market photography to Training Tools with visual examples for planning, mobile review and chart analysis.
- Added real-life market photography to A–Z Trading Jargon so definitions are framed in recognisable market/trading settings.
- Reduced repeated News fallback images by choosing among the proven market photo set deterministically from each story headline instead of using one fixed image per category.
- Existing company logo fixes, News editorial-image upgrades, rich Home explainers and mobile Home swipe behaviour are retained.

### v103 changes
- Changed strategy for the repeatedly failing company icons: Chevron, Marvell Technology, Micron, Vertex and PDD now ship with local SVG brand assets inside the MOVA package.
- These local files load before any CDN, favicon or company-website logo lookup, so they no longer depend on an external logo service.
- News image rendering is now deterministic: every card displays a real photographic image immediately instead of a text placeholder or screenshot.
- If the feed already contains a non-generic story image it is shown first; otherwise MOVA uses a proven market photograph immediately.
- The article's own Open Graph image can still upgrade the card after load if it is a genuine large editorial image.
- Article screenshots and giant text-image placeholders are no longer part of the News rendering path.
- v101 mobile Home-carousel swipe fix and v99 rich Home explainers are retained.

### v102 changes
- Fixed the remaining recurring letter-only company-logo cases shown in testing by adding reliable Wikimedia-hosted brand assets for Chevron (CVX), Marvell Technology (MRVL), Micron (MU), Vertex Pharmaceuticals (VRTX) and PDD/Pinduoduo (PDD).
- These Wikimedia assets are tried before the dynamic website/logo services, so the affected companies no longer depend on a favicon service returning the correct brand mark.
- Reduced the visual prominence of the generic letter tile; it remains only as the final fallback if every real logo source fails.
- Removed article-page screenshots from News image fallback. Those screenshots were responsible for the giant text-image panels such as “Which dow” and “Oil climbs”.
- News cards now use: real feed image → real article Open Graph image → relevant photographic market fallback.
- Added relevant photographic fallbacks for Nasdaq/technology, commodities/oil, gold, macro/current affairs and general market stories.
- News cards display a subtle loading shimmer while resolving an image instead of large placeholder text.
- v101 mobile Home-carousel swipe fix is retained unchanged.
...
## Vercel recovery note

This recovery package adds `vercel.json` at the project root.

It explicitly rewrites `/` to `/index.html`, which prevents the Vercel `404: NOT_FOUND`
screen if the deployment is no longer automatically resolving the root document.

When uploading/replacing files, keep these items at the project root:
...
Do not place all of them inside an extra nested folder.


## Recovery package correction — complete API folder

The earlier recovery package accidentally omitted two API endpoints that `index.html`
still calls:
...
The complete API folder now contains:
...
- `api/fundamentals.js`
- `api/news.js`
- `api/company-visual.js`
- `api/news-visual.js`

A package-level verification was run to confirm every `/api/...` reference in
`index.html` has a matching JavaScript endpoint file.
